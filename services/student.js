...
class StudentService {
  ...
  static async searchBy(search = '', { offset = 0, limit = this.QUERY_LIMIT_SIZE}) {
    ...
    let docs = await StudentModel.find({ $or: [{department_serial_number: pattern}, {first_name: pattern}, {surname: pattern}, {middle_name: pattern}]}).skip(offset).limit(limit).sort('-_id')
    
    return docs
  }
  
  static async findAll({ offset = 0, limit = this.QUERY_LIMIT_SIZE}) {
    return StudentModel.find().skip(offset).limit(limit).sort('-_id')
  }
